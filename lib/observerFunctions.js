...
export const getPosts = async (from, last = false) => {
    let response = await axios.post("/api/posts", { last, from }).then(response => { return response.data })
    return response
}

export const timePosts = (time) => {
    let timeReal = time * 1000
    return new Date(timeReal).toLocaleString()
}